...
export default function PostBoard() {
    ...
    return (
        <div className="flex flex-col items-center">
            <h2 className="text-3xl font-[600] w-full border-b-2 bg-white p-5">My posts / {typeList.map((type) => pathname.includes(type.path) ? <Link to={type.path}>{type.content}</Link> : '')}</h2>

            <ul className="flex gap-5 border-primary border-t-1 p-10 w-full">
